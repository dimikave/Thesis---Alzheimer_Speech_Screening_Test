...
for df in dfs:
    ...
    for name in Names_test:
        question_name = name.rpartition('_')[0]
        question_name = question_name.rpartition('_')[2]
        
        question_list.append(question_name)
        
        
        # print(question_name)
        if question_name not in question_hit_dict:
            question_hit_dict[question_name] = 0
        if y_pred[j]==Y_test[j]:
            question_hit_dict[question_name] = question_hit_dict[question_name]+1
    ...
    counts = pd.Series(question_list).value_counts()
    counts = counts.to_dict()
    count_quest.update(counts)
    # print(counts)
    # print(question_hit_dict)
    question_scores = {}
    for key in question_hit_dict:
        question_scores[key] = question_hit_dict[key]/counts[key] * 100
    ...
    if ET:
        ...
        # print(question_scores)
        q_scores_r.update(question_scores)
    else:
...
print('\n\n\n')
count_quest
last_dict = {'Hit Ratio': list(q_scores.values()), 'Counter': list(count_quest.values())}
q_scores = pd.DataFrame(data=last_dict, index= list(q_scores.keys()))
last_dict = {'Hit Ratio': list(q_scores_r.values()), 'Counter': list(count_quest_r.values())}
q_scores_r = pd.DataFrame(data=last_dict, index= list(q_scores_r.keys()))
